...
export default function Departments() {
  ...
  const load = () => authedFetch('/catalog/departments').then(r=>r.ok?r.json():[]).then(setRows).catch(()=>{});
  useEffect(() => { load(); }, []);

  const openCreate = () => { setEditingId(null); setForm(BLANK); setShowForm(true); };
  const startEdit  = (d) => { setEditingId(d.id); setForm({ name:d.name||'', code:d.code||'' }); setShowForm(true); };

  const submit = async () => {
    if (!form.name.trim()) return alert('Department name required');
    setSaving(true);
    try {
      const url = editingId ? `/catalog/departments/${editingId}` : '/catalog/departments';
  ...
  const remove = async (d) => {
    ...
    await authedFetch(`/catalog/departments/${d.id}`, { method:'DELETE' });
    load();
  };
    ...
    <div>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'2rem' }}>
        <div>
          ...
          <h1 style={{ fontFamily:'Manrope,sans-serif', fontSize:'2rem', fontWeight:800, color:'#0f1218', letterSpacing:'-0.025em' }}>Departments</h1>
          <p style={{ color:'#8892a4', fontSize:'0.82rem', marginTop:'0.2rem' }}>{rows.length} departments · lab sections (Biochemistry, Haematology, …)</p>
        </div>
      ...
      {showForm && (
        <div style={{ ...S.card, marginBottom:'1.5rem', border:'1px solid rgba(249,115,22,0.2)' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0.9rem', marginBottom:'1rem' }}>
            <div><label style={lbl}>Department Name *</label>
              <input style={inp} placeholder="e.g. Biochemistry" value={form.name} onChange={e=>setForm({...form,name:e.target.value})} /></div>
            <div><label style={lbl}>Code</label>
              <input style={inp} placeholder="e.g. BIOCHEM" value={form.code} onChange={e=>setForm({...form,code:e.target.value})} /></div>
          </div>
      ...
      <div style={{ ...S.card, padding:0, overflow:'hidden' }}>
        <table style={{ width:'100%', borderCollapse:'collapse' }}>
          ...
          <tbody>
            {rows.length === 0 && (
              <tr><td colSpan={3} style={{ textAlign:'center', padding:'3rem', color:'#8892a4' }}>No departments yet.</td></tr>
            )}
            {rows.map(d => (
              <tr key={d.id} style={{ borderBottom:'1px solid #f4f6fa' }}
                onMouseEnter={e=>e.currentTarget.style.background='#fafbfc'}
                onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
                <td style={{ padding:'0.9rem 1.3rem', fontWeight:700, color:'#0f1218', fontSize:'0.88rem' }}>{d.name}</td>
                <td style={{ padding:'0.9rem 1.3rem', color:'#8892a4', fontSize:'0.82rem', fontFamily:'monospace' }}>{d.code || '—'}</td>
                <td style={{ padding:'0.9rem 1.3rem' }}>
                  <div style={{ display:'flex', gap:'0.4rem' }}>
                    <button title="Edit" onClick={()=>startEdit(d)} style={iconBtn('#2563eb')}>
                      <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg>
                    </button>
                    <button title="Delete" onClick={()=>remove(d)} style={iconBtn('#dc2626')}>
